      ...
      <div class="card-body">
        ...
        <div class="row mb-2">
          <div class="col-sm-8">
            ...
            <a href="{{ route('admin.domains.index') }}" class="mr-2 btn btn-outline-primary @if($type=="all") active @endif">{{ __('All') }} ({{ $all }})</a>

            <a href="{{ route('admin.domains.show',1) }}" class="mr-2 btn btn-outline-success @if($type==1) active @endif">{{ __('Active') }} ({{ $actives }})</a>

            <a href="{{ route('admin.domains.show',2) }}" class="mr-2 btn btn-outline-danger @if($type==2) active @endif">{{ __('Expired') }} ({{ $drafts }})</a>

            <a href="{{ route('admin.domains.show',3) }}" class="mr-2 btn btn-outline-warning @if($type==3) active @endif">{{ __('Requested') }} ({{ $Requested }})</a>


            <a href="{{ route('admin.domains.show',0) }}" class="mr-2 btn btn-outline-danger @if($type== 0 && $type != 'all') active @endif">{{ __('Trash') }} ({{ $trash }})</a>
          </div>

          <div class="col-sm-4 text-right">
{{--            @can('domain.create')--}}
            <a href="{{ route('admin.domains.create') }}" class="btn btn-primary">{{ __('Create Domain') }}</a>
{{--            @endcan--}}
          </div>
        </div>
            ...
            <table class="table table-striped table-hover text-center table-borderless">
              ...
              <tbody>
                ...
                <tr id="row{{ $row->id }}">
                  ...
                  <td><a @if(Route::has('admin.customers.index')) href="{{ route('admin.customers.show',$row->user->id) }}" @endif>{{ $row->user->name }}</a></td>
                  <td>
                  ...
                  <td>
{{--                    @can('domain.edit')--}}
                    <a href="{{ route('admin.domains.edit',$row->id) }}" class="btn btn-primary btn-sm text-center"><i class="far fa-edit"></i></a>
{{--                    @endcan--}}
